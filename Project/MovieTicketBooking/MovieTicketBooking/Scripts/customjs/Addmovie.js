...
function validateDuration() {
    ...
    // Check if the duration is an integer and between 60 and 300
...
function validateMoviePoster() {
    var moviePosterInput = $('#moviePoster');
    var errorSpan = $('#errorMoviePoster');
    var isValid = true;

    if (!moviePosterInput.val()) {
        errorSpan.text("Please select a movie poster.").show();
        isValid = false;
    } else {
        errorSpan.text("").hide();
    }
...
function validateActor() {
    ...
    // 3 to 20 characters, letters and spaces allowed, no leading or trailing spaces
...
$('#addMovieForm').on('submit', function (event) {
    ...
    var isMoviePosterValid = validateMoviePoster();
    ...
    if (!isMovieNameValid || !isDescriptionValid || !isDurationValid || !isMoviePosterValid || !isActorValid || !isActressValid || !isDirectorValid) {
