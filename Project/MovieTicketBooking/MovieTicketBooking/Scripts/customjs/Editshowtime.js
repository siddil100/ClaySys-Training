$(document).ready(function () {
    ...
    tenDaysFromTomorrow.setDate(tomorrow.getDate() + 10); // 10 days from tomorrow

    var minDate = tomorrow.toISOString().split('T')[0]; // Format: YYYY-MM-DD
    var maxDate = tenDaysFromTomorrow.toISOString().split('T')[0]; 
    ...
    $('#StartTime').on('change', function () {
        ...
        if (startTime === "00:00" || (hours >= 0 && hours < 4)) {
            ...
            $('#errorStartTime').text("Start time cannot be 00:00 and hours from 00:00 to 03:59 are not allowed.").show();
        } else {
    ...
    $('#editShowtimeForm').on('submit', function (event) {
        ...
        if (!isStartTimeValid) {
            $('#errorStartTime').text("Please select a valid start time between 04:00 and 12:00.").show();
        } else {
            $('#errorStartTime').text("").hide();
        }
